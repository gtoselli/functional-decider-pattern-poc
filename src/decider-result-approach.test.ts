...
import { createDeciderWithResultRunner, decider } from './decider-result-approach';

describe('result decider approach', () => {
  const INITIAL_STATE = {
    id: 'foo-id',
    paths: [],
  };
  const runner = createDeciderWithResultRunner(decider, INITIAL_STATE);
  ...
  describe('Given a patient with an active wlm path', () => {
    ...
    it('When start with same path, should fail', () => {
      ...
      if (result.isErr()) expect(result.error).toEqual('PATH_ALREADY_EXISTS');
    });
    ...
    it('When addAnotherProfessional with nutritionist role, should return failure', () => {
      ...
      expect(result.isErr()).toBeTruthy();
      if (result.isErr()) expect(result.error).toEqual('PROFESSIONAL_ROLE_ALREADY_EXISTS');
    });
    ...
    describe('Given two professionals', () => {
      ...
      it('When addAnotherProfessional, should return failure', () => {
        ...
        expect(result.isErr()).toBeTruthy();
        if (result.isErr()) expect(result.error).toEqual('MAX_PROFESSIONALS_REACHED');
      });
    });
    ...
    describe('Given a scheduled session', () => {
      ...
      it('When schedule a session before existing session, should update sequences', () => {
        ...
        expect(result.value[0].type).toBe('SESSION_SCHEDULED');
        expect(result.value[1].type).toBe('SESSIONS_SEQUENCE_CHANGED');
  ...
  describe('Given a patient with an active psychotherapy path', () => {
    ...
    it('When addAnotherProfessional, should return failure', () => {
      ...
      expect(result.isErr()).toBeTruthy();
      if (result.isErr()) expect(result.error).toEqual('PATH_DOES_NOT_SUPPORT_MULTIPLE_PROFESSIONALS');
    });
  });
});
